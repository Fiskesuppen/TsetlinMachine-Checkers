...
def tsetlinStandardHandler(epochs, clauses, T, s, k_fold_amount):
    score = tsetlinStandard(k_fold_amount, clauses, T, s, epochs)
    print(score)
    average_score = round(sum(score)/len(score),2)

    #acstr = str(accuracy)
    with open("StandardTsetlinLog.txt", "a+") as myfile:
